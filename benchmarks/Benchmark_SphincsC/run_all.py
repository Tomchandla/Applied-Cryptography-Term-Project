...
import bench_common_C as bench_common_C
import bench_keygen_C as bench_keygen_C
import bench_sig_size_C as bench_sig_size_C
import bench_verification_C as bench_verification_C
import bench_signing_C as bench_signing_C
from sphincs.sphincs_Plus_C import SphincsParamsC
# constraints
MAX_SIG_BYTES = 2_000_000 # maximum signature size to consider, filter out anything with too large a sig, moddable.
...
RUNNERS = {
    "keygen": bench_common_C.run_verify_sphincs_plus_c,
    "sign":   bench_common_C.run_sign_sphincs_plus_c,
    "verify": bench_common_C.run_verify_sphincs_plus_c,
    "sig_size": bench_common_C.run_sig_size_sphincs_plus_c,
}
...
def print_top(results: list, curr_op: str,  n: int = 10):
    ...
    print(f"  {'rank':>4}  {'n':>2} {'w':>3} {'h':>2} {'d':>2} "
          f"{'k':>2} {'t':>2} {'t\'':>4} {'z':>4}  {'sig(B)':>7}  {curr_op}(ms)")
    print(f"  {'─' * 74}")
    ...
    bench_keygen_C.print_results(ranked[0]["raw"])           if curr_op == "keygen" else \
    bench_signing_C.print_results(ranked[0]["raw"])          if curr_op == "sign"   else \
    bench_verification_C.print_results(ranked[0]["raw"])     if curr_op == "verify" else \
    bench_sig_size_C.print_results(ranked[0]["raw"])


def run():
    # uncomment below if OS is unix to memory limit param opt test.
    # set_memory_limits()
    print(f"Welcome to param optimizer: Sphincs+C")
    for op in ALL_OPS:
        print (f"Currently running: {op}")
        results = sweep(op)
        if results:
            print_top(results, op, n=10)
            file_path =  os.path.join(script_dir, f"results_{op}_C.csv")
