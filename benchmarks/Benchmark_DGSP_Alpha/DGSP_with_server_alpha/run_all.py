...
import bench_common_serv_DGSP_alpha as bench_common
import bench_join_serv_DGSP_alpha
import bench_keygen_serv_DGSP_alpha
import bench_respM_serv_DGSP_alpha
import bench_judge_serv_DGSP_alpha
import bench_open_serv_DGSP_alpha
import bench_verify_serv_DGSP_alpha
import bench_sign_serv_DGSP_alpha

from params.sphincs_params_Alpha import SphincsParamsAlpha

SERVER_PY = os.path.join(os.path.dirname(__file__), "bench_server_alpha.py")
...
def wait_for_server(params: SphincsParamsAlpha, timeout: float = 15.0) -> bool:
...
def print_top(results: list, curr_op: str, n: int = 10) -> None:
    ...
    elif curr_op == "open":     bench_open_serv_DGSP_alpha.print_results(ranked[0]["raw"])
    elif curr_op == "sign":     bench_sign_serv_DGSP_alpha.print_results(ranked[0]["raw"])
    elif curr_op == "verify":   bench_verify_serv_DGSP_alpha.print_results(ranked[0]["raw"])
...
def run():
    ...
    for idx, (n, w, h, d, k, t) in enumerate(combos, 1):
        ...
        server_temp_params = SphincsParamsAlpha(n, w, h, d, k ,t)
