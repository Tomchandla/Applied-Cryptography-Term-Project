"""
SPHINCS+ Alpha Parameter Optimisation (using existing benchmark runners)
============================================================
...
import bench_common_alpha as bench_common_alpha
import bench_keygen_alpha as bench_keygen_alpha
import bench_sig_size_alpha as bench_sig_size_alpha
import bench_verification_alpha as bench_verification_alpha
import bench_signing_alpha as bench_signing_alpha
...
RUNNERS = {
    "keygen": bench_common_alpha.run_keygen_sphincs_alpha,
    "sign":   bench_common_alpha.run_sign_sphincs_alpha,
    "verify": bench_common_alpha.run_verify_sphincs_alpha,
    "sig_size": bench_common_alpha.run_sig_size_sphincs_alpha,
}
...
def print_top(results: list, curr_op: str, n: int = 10):
    ...
    bench_keygen_alpha.print_results(ranked[0]["raw"])        if curr_op == "keygen" else \
    bench_signing_alpha.print_results(ranked[0]["raw"])       if curr_op == "sign"   else \
    bench_verification_alpha.print_results(ranked[0]["raw"])  if curr_op == "verify" else \
    bench_sig_size_alpha.print_results(ranked[0]["raw"])


def run():
    # uncomment below if OS is unix to memory limit param opt test.
    # set_memory_limits()
    print(f"Welcome to param optimizer: Sphincs+ Alpha")
    for op in ALL_OPS:
        print (f"Currently running: {op}")
        results = sweep(op)
        if results:
            print_top(results, op, n=10)
            file_path =  os.path.join(script_dir, f"results_{op}_alpha.csv")
